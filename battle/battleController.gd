...
func _ready():
	...
	while iterateSpawnTeam < team.size():
		...
		var character = load("res://battleAsset/chra/Player" + String(iterateSpawnTeam + 1) + "Battle.tscn").instance()
	...
	while iterateSpawnEnemy < enemies.size():
		var enemy = load("res://battleAsset/enemy/" + enemies[iterateSpawnEnemy] + ".tscn").instance()
		...
		match menuIndex:
			1:
				...
				while createPerEnemy < enemies.size():
					var enemyStuffBox = preload("res://battleAsset/EnemyName.tscn").instance()
					...
					enemyStuffBox.get_node("SPARE_bar/bar_percent").text = String(floor(float(enemyStat[createPerEnemy][2]) / 100.0) * 100) + "%"
					createPerEnemy += 1
